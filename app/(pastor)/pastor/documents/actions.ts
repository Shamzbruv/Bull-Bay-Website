...
export async function certifyDocument(requestId: string): Promise<ActionState> {
 return officeAction(async () => {
  ...
  try {
   ...
   await notifyUsers(org,pastors.flatMap(p=>p.auth_user_id && p.auth_user_id!==user.id?[p.auth_user_id]:[]),{title:"Your signature and the church stamp were used",body:`${profile.first_name ?? ""} ${profile.last_name ?? ""} certified ${request.title} (${numbered.document_number}).`,url:"/pastor/documents",type:"document_signature"});
