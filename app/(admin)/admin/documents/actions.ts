...
export async function prepareRequest(id:string,_:ActionState,form:FormData):Promise<ActionState>{return officeAction(async()=>{
 ...
 const pastors=await roleMembers(org,['pastor']);await notifyUsers(org,pastors.flatMap(p=>p.auth_user_id?[p.auth_user_id]:[]),{title:"A document is ready for your approval",url:`/pastor/documents?request=${id}`,type:"document"});return "Sent to the Pastor for review. Authorized signers can now certify it.";
});}
